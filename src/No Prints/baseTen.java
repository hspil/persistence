		...
		try {
			here: while(true){ // The "here:" is for later, establishing a functional loop
				...
				if (stringTotal.charAt(0) != '2' && stringTotal.charAt(0) != '3'){ // We don't need numbers that don't start with 2 or 3 - Well, probably
					if(stringTotal.charAt(0) == '1'){
						nums = nums.add(new BigInteger(thing.giveMe(stringTotal.length() - 1)));	
						continue here;				
					}
					nums = nums.add(new BigInteger(thing.giveMe(stringTotal.length() - 1, Integer.toString(12 - Integer.parseInt(stringTotal.charAt(0) + ""))))); // Adds 1 to the first digit
					continue here;
				}
				for (int t = 0; t < stringTotal.length(); t++) {
					if (temp > Character.getNumericValue(stringTotal.charAt(t))){
						nums = nums.add(new BigInteger(thing.giveMe(stringTotal.length() - t - 1, Integer.toString(temp - Character.getNumericValue(stringTotal.charAt(t))))));
						continue;
					}
					...
					if (temp == 2) // These are for more conditionals
						twos++;
					if (temp == 3)
						threes++;
					if (temp == 6)
						sixes++;
					if (temp == 7) // These are for more conditionals
						sevens++;
					if (temp == 8)
						eights++;
					if (temp == 9)
				...
				if(baseTen.ratio(sevens, eights, nines) == true){
