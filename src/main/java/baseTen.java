...
public class baseTen {
    public static void main(String[] args) throws IOException {
        ...
        try {
            here:
            while (true) { // The "here:" is used to return to the beginning of the while(true) loop.
                ...
                for (int t = 0; t < stringTotal.length(); t++) {
                    if (temp > (stringTotal.charAt(t) - '0')) {
                    ...
                    switch (stringTotal.charAt(t)) { // Skips if there's a 1, 4, 5, or 0. In these cases, the number is not the first number (which can be 2 or 3), so we add the difference to get to 6.
                        case '0': // Obviously, this has a persistence of 1.
                        case '4': // 4s are evil
                            nums = nums.add(new ArbitraryInteger(thing.giveMe(stringTotal.length() - t - 1, "2")));
                            continue here;
                ...
                if (stringTotal.length() > currLeng) {
                    ...
                    System.out.println("Current Max Persistence: " + MAXSTEPS + " == " + MAXSTEPS_NUMBER + " Number length: " + MAXSTEPS_NUMBER.length() + " Current Length: " + currLeng + " (Total Time: " + thing.timeFormat(nowTime - initialTime) + " (Action Time: " + thing.timeFormat(nowTime - actTime) + " Threads: " + threadRipper.numThreads + " Thread difference: " + (threadRipper.numThreads - numThreads));
            ...
            writer.close();
            System.out.println("Exception caught during while(true) loop: " + e.toString());
        }
    }
